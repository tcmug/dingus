...
int main(int argc, char *args[]) {
  ...
  printf("\n\n");
  {
    ...
    real d = TW_SphereSweepTriangle(s, v, t, &TW_Vector2);
    TW_Vector3 dest =
        TW_Vector3AddVector(s.origin, TW_Vector3MulReal(TW_Vector3Normalize(v), d));
    printf("Tri Sweep d: %f\n", d);
  ...
  while (running) {
    ...
    while (SDL_PollEvent(&ev)) {
      ...
      case SDL_MOUSEBUTTONDOWN: {
        ...
        app_log("TW_Component at TW_Vector2 (%i, %i) is %p", TW_Vector2.x, TW_Vector2.y, self);
        if (self->click) {
    ...
    for (int i = 0; i < num_spheres; i++) {
      ...
      while (TW_Vector3Length(col.velocity) > 0.0001) {
        if (TW_TestTriangles(&col, num_triangles, triangles)) {
          ...
          TW_Vector3 normal =
              TW_Vector3Normalize(TW_Vector3SubVector(s->origin, col.TW_Vector2));
